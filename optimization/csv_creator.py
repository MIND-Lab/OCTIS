...
def save_csv(name_csv, dataset_name, hyperparameters_name, metric_name, surrogate_model,
             acquisition_function, times,  # list of list of time
             res, maximize=False, time_x0=None, len_x0=0):
    """
        ...
        surrogate_model : [string] surrogate model used

        acquisition_function : [string] acquisition function used

        times : [list of list] Of time for each point evaluated
        ...
        time_x0 : [list of list] If not-None it adds x0 and y0 time to the csv file
        """
    ...
    name_dict['SURROGATE'] = [surrogate_model] * n_row
    name_dict['ACQUISITION FUNC'] = [acquisition_function] * n_row
    ...
    for i in range(n_point):
        ...
        name_dict['TIME'].append(times[i])  # time
