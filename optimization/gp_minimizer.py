...
# GP Minimize
def gp_minimizer(f, bounds, number_of_call, kernel, acq_func,
                 random_state, noise_level,  # attenzione
                 alpha, x0, y0, time_x0, n_random_starts,
                 save, save_step, save_name, save_path, early_stop, early_step,
                 plot_best_seen, plot_prefix_name, log_scale_plot,
                 verbose, model_queue_size, dataset_name, hyperparameters_name,
                 metric_name, maximize):
    res = []
    surrogate_model_name = "gp_minimize"
    ...
    if not save and not early_stop:
        ...
        opt = skopt_optimizer(bounds, base_estimator=gpr, acq_func=acq_func,
                              n_random_starts=n_random_starts, n_initial_points=n_random_starts,
                              acq_optimizer="sampling", random_state=random_state,
        ...
        if save:
            save_csv(name_csv=save_path + save_name + ".csv", dataset_name=dataset_name,
        ...
        while number_of_call_r > 0:
            if number_of_call_r >= save_step:
                ...
                opt = skopt_optimizer(bounds, base_estimator=gpr, acq_func=acq_func,
                                      n_random_starts=0, n_initial_points=0,
                                      acq_optimizer="sampling", random_state=random_state,
                                      model_queue_size=model_queue_size)
            ...
            if number_of_call_r >= save_step:
                ...
                if not early_stop_flag:
                    ...
                    save_csv(name_csv=save_path + save_name + ".csv", dataset_name=dataset_name,
                             hyperparameters_name=hyperparameters_name, metric_name=metric_name,
                             surrogate_model=surrogate_model_name, acquisition_function=acq_func,
                             times=time_eval, res=res,
                             maximize=maximize, time_x0=time_x0, len_x0=lenx0)
