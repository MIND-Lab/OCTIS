...
class BestEvaluation:
    ...
    def save_to_csv(self, name_file):
        ...
        df['Mean(model_runs)'] = [np.mean(self.dict_model_runs[self.metric.__class__.__name__]['iteration_' + str(i)])
                                  for i in range(n_row)]
        df['Standard_Deviation(model_runs)'] = [
            np.std(self.dict_model_runs[self.metric.__class__.__name__]['iteration_' + str(i)]) for i in range(n_row)]
        ...
        for metric, i in zip(self.extra_metrics, range(n_extra_metrics)):
            try:
                df[metric.info()["name"] + '(not optimized)'] = [
                    np.median(self.dict_model_runs[metric.__class__.__name__]['iteration_' + str(i)]) for i in
                    range(n_row)]
            except:
