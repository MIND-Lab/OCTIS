...
from octis.models.rl_for_topic_models.networks.inference_network import InferenceNetwork
...
class DecoderNetwork(nn.Module):
    ...
    def __init__(self, input_size, bert_size, n_components=10,
                 ...
                 policy_dropout=0.0, kl_multiplier=1.0):
        """
        ...
        Args
            ...
            kl_multiplier : float or int, multiplier on the KL divergence (default 1.0)
        """
        ...
        self.mu_inference = InferenceNetwork(
            bert_size, n_components, hidden_sizes, activation=activation, dropout=inference_dropout)
    ...
    def loss_fn(self, bow, word_dist, posterior_mu, posterior_log_sigma, epsilon=1e-8):
    ...
    def forward(self, x_bow, x_bert):
        ...
        posterior_log_sigma = self.posterior_log_sigma_norm(posterior_log_sigma_unnormalized)
        posterior_distribution = Normal(posterior_mu, torch.exp(posterior_log_sigma))

        # RL policy
        action = posterior_distribution.rsample()
        self.action = action
        policy = (1 / (torch.exp(posterior_log_sigma) * math.sqrt(2 * math.pi))) \
            * torch.exp(-1.0 * (action - posterior_mu) ** 2 / (2 * torch.exp(posterior_log_sigma) ** 2))
        policy = self.dropout(policy)

        # product of experts
        word_dist = F.softmax(self.beta_norm(torch.matmul(policy, self.beta)), dim=-1)
